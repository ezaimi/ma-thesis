...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    % planned, not-yet-implemented feedback path (kept shallow so its label
    % stays clear of the SQLite/KG output boxes further below)
    \draw[darr] (fa.south) .. controls +(0,-9mm) and +(0,-9mm) .. (ra.south)
      node[midway, below, font=\scriptsize] {planned bounded second round (not yet implemented)};
    ...
    \node[font=\scriptsize, anchor=south] at (le.north |- orch.north)
      {Orchestrator (sequences the five components; design target, not yet built)};
  \end{tikzpicture}%
  }
  \caption{Component architecture of the repair layer. The Docker pipeline's
           ...
           which is a design target and not yet a built component
           (\Cref{sec:orchestrator}); RAGRepairAgent queries PyPI for live
           metadata; ResultLogger writes both the SQLite benchmark table and
           the FAIR Jupyter knowledge graph. The dashed feedback arrow is the
           planned bounded second round, also not yet implemented.}
...
\fullrunupdate{This orchestrator is a design target, not yet a built
component. Each of ErrorClassifier through ResultLogger exists and is
independently tested (\Cref{chap:methodology}); the loop described below
that would run them together, unattended, over the full 200-row usable set
has not been implemented at the time of writing. \Cref{chap:future-work}
lists full orchestration and the two-round feedback mode as remaining work;
result logging and knowledge-graph enrichment are already implemented
(\Cref{sec:m-resultlogger}) and are not part of what remains here.}
...
\begin{itemize}
  ...
  \item Full end-to-end orchestration and the two-round feedback mode --
        \textbf{not yet implemented}; see \Cref{chap:future-work}.
\end{itemize}
