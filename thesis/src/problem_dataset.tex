...
\section{Scale of the Problem (L1)}
\label{sec:pd-l1}
...
\section{The Working Dataset (I1)}
\label{sec:pd-i1}
...
produced by \texttt{scripts/prepare\_dependency\_dataset.py} (implementation
step I1), and is written out as a CSV file plus a machine-readable statistics
file so that the numbers can be independently recomputed.
...
\begin{table}[h]
  ...
  \begin{tabular}{|l|r|p{7.5cm}|}
    ...
    \hline
    \texttt{evaluation} & 187 & Usable rows reserved for the eventual full-scale repair evaluation (\Cref{chap:future-work}); not yet run at the time of writing. \\
    \hline
    \texttt{excluded} & 14 & Outside the pip-only v1 repair scope; still explained, never repaired (\Cref{sec:pd-explanation-scope}). \\
    \hline
  \end{tabular}
...
It is important not to read the 187-row \texttt{evaluation} split as already
evaluated. Splitting the data this way only \emph{reserves} those rows for a
later, controlled repair run; at the implementation stage this thesis
reports, that run has not yet happened (\Cref{chap:future-work}).

\subsection{Explanation Scope Is Wider Than Repair Scope}
\label{sec:pd-explanation-scope}

One further decision, made explicit only while building the repair
component in step I4, deserves stating clearly here because it affects every
later chapter. The pip-only restriction above governs \emph{repair}
eligibility only. It does not restrict \emph{explanation}. All 214 rows,
including the 14 excluded ones, still receive a plain-language explanation
of their failure (objective O1); only the 200 usable rows are additionally
passed on to the repair-proposal and fix-application components (objectives
O2 and O3). A researcher whose notebook fails because of a missing system
...
scope and 13 of those were used only for prompt design, leaving 187 rows
reserved, but not yet used, for a full evaluation. Keeping these numbers
