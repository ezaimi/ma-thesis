...
\fullrunupdate{Each pilot exercises one component's own stages against its
relevant real external system; no single unattended run has yet driven all
components together over the dataset (\Cref{sec:orchestrator}). Neither kind
of evidence is the thesis's final evaluation, which requires running the
full 187-row evaluation split described in \Cref{chap:problem-dataset}, and
that full evaluation run has not yet been performed
...
achievable in principle on a single-dependency notebook, or with the future
two-round repair mode feeding a \texttt{still\_failing} result's new error
back into a second repair attempt (\Cref{chap:future-work}), but neither has
...
% TODO-FULL-RUN: update after full orchestrated dataset execution
\fullrunupdate{This is component-by-component pilot validation: no single
unattended run has yet driven all components together over the dataset, and
that remains the job of the orchestrator described in
\Cref{chap:architecture,chap:future-work}.}
