...
Full pipeline orchestration -- a single process that would run every
component above together, unattended, over the complete dataset -- is not
covered in this chapter because it has not been built yet; it is addressed
directly in \Cref{chap:future-work}.
...
Each arrow above is a data hand-off through a plain file or database, not a
function call inside one running program: every stage is invoked as its own
script against the previous stage's output, and no single process currently
runs all of them in one pass over the whole dataset --
\Cref{sec:orchestrator,chap:future-work} return to this point directly. The
...
% TODO-FULL-RUN: update after full orchestrated dataset execution and completion of the bounded second repair round
...
RAGRepairAgent for a second attempt remain explicitly future work
...
abstained or failed attempt to be logged regardless. None of the earlier
components were changed to accept a shared, externally supplied
\texttt{run\_id} for this -- each still mints its own per invocation; a
single orchestration-level run identifier threaded through every stage of
one pipeline pass is left to full pipeline orchestration
(\Cref{chap:future-work}), once it actually calls every stage in sequence
within one pass.
