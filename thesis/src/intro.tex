...
\begin{itemize}
  ...
  \item Objective O5, full pipeline integration, is \textbf{designed but not
        yet built}: there is currently no single program that runs all five
        repair-layer components over the whole dataset automatically.
        % TODO-FULL-RUN: update after full orchestrated dataset execution
        \fullrunupdate{The benchmark table and the knowledge graph so far
        hold only pilot-scale records rather than the complete dataset.}
