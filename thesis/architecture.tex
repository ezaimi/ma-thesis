...
Following the taxonomy of Yang et al.~\cite{yang2025survey}, % CITE [15]
...
al.~\cite{grotov2024untangling,grotov2024debug} % CITE [7],[8]
...
The containerised FAIR Jupyter pipeline~\cite{samuel2025docker} % CITE [1]
...
\texttt{url} (and \texttt{error\_category}).

\paragraph{Working set.}
The committed Docker-run database yields on the order of two hundred
\texttt{DEPENDENCY\_ERROR} targets. A small number ($\approx 10$) are
system-library failures (missing \texttt{.so} objects) that require
\texttt{apt-get} rather than \texttt{pip} and fall outside the pip-based
fix scope of this thesis; the remainder are pip-fixable. The larger
reproducibility figures from the complete conda
run~\cite{samuel2024gigascience} % CITE [2]
...
relevant cell).\\
\textbf{Output.} An explanation string.\\
...
the fix is correct~\cite{grotov2024debug}. % CITE [8]\\
...
ChatRepair~\cite{xia2024chatrepair}; % CITE [12]
...
The FAIR Jupyter knowledge graph~\cite{samuel2024fairjupyter} % CITE [3]
...
FAIR4RS~\cite{barker2022fair4rs}. % CITE [N10], [N11]
